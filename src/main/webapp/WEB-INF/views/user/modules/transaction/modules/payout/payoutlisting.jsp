...
<ul class="inline">
	<c:forEach var="stripe" items="${stripes}">
		<c:set var="p" value="${stripe.accountAdapter.cardAdapter }" />
		...
		<spring:url var="deletePayout" value="/user/delete-a-payout" htmlEscape="true"/>
		<spring:url var="setPayoutDefault" value="/user/set-default-a-payout" htmlEscape="true"/>
	
			<li id="${pid}Row">
			<div class="cube">
				<div class="cubeHeader">
					<form class="setPayoutDefault toRight" action="${setPayoutDefault}" method="POST">
				...
				<div class="cubeFooter">
					...
					<form class="deletePayout toRight" action="${deletePayout}" method="POST">
						<input type="hidden" value="${pid }" name="_id">
...
$(document).ready(function() {
	...
	$("form.setPayoutDefault").on("submit", function(event){
	...
	$("form.deletePayout").on("submit", function(event){
		var serializedData = $(this).serialize();
		var urlForm = $(this).attr("action");
		var rowId = $(this).find("input[name=_id]").val();
		var $rowId = $("#" + rowId + "Row");
