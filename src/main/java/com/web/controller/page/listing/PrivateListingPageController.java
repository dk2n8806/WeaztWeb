...
@Controller
public class PrivateListingPageController 
	extends AbstractMerchantBaseController 
{
	...
	@RequestMapping(value=UriPageRequestMapping.Merchant.Listing.PRIVATE_LISTING
											...
											, @RequestParam(name="s", defaultValue="20") int pageSize) 
	{
		...
		long listingSize = 0;
		ProductStatus status = ProductStatus.PRIVATE;
		...
		try {
			...
			listingSize = productService.countByMerchant(merchant, category, status, dateInteval);	
		
			if(listingSize > 0) {
				products = templateService.getByMerchant(merchant, category, status, dateInteval, pageable);
			}
		...
		return PageAdvice.Merchant.Listing.PRIVATE_LISTING;
	}
